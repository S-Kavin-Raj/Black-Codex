...
function calculateNetworkScore(scanId = null) {
  ...
  if (!scores.length) return { networkScore: 100, deviceScores: [] };
  // Aggregate: average device scores
  const networkScore = Math.round(scores.reduce((sum, s) => sum + s.score, 0) / scores.length);
  return {
    networkScore,
    deviceScores: scores
  };
