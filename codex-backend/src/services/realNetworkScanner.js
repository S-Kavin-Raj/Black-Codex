...
async function fullNetworkScan(subnet, progressCallback) {
  ...
  if (!subnet) {
    ...
    if (networkInfo.length > 0) {
      subnet = calculateSubnet(networkInfo[0].ip, networkInfo[0].netmask);
      logger.info(`Auto-detected subnet: ${subnet}`);
    } else {
    ...
    try {
      ...
      if (existing) {
        ...
        `).run(
          ...
          device.ip,
          device.mac,
          `New device discovered: ${device.hostname || device.ip} (${device.vendor})`,
          now
        );
      }
