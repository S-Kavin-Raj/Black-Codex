...
const IOT_PATTERNS = [
    // IoT Chips & Modules
    /espressif/i, /esp32/i, /esp8266/i, /esp/i,
    ...
    // Network Equipment (IoT context)
    /tp-?link/i, /d-?link/i, /netgear/i, /linksys/i,
    /shenzhen.*zero/i, /zeroone/i,  // Routers/Gateways
    // Smart Devices
    ...
    /ncomputing/i, /wyse/i,
    // Printers (Network IoT)
    /hp.*printer/i, /canon/i, /epson/i, /brother/i,
    // Smart Power & Components
    /liteon/i, /lite-?on/i,
    // DIY / Maker
    /raspberry.*pi/i, /arduino/i,
    // Generic IoT indicators
    /\biot\b/i, /sensor/i, /camera/i, /cctv/i, /nvr/i, /dvr/i
];
...
async function getVendorFromNmap(ip) {
    try {
        // Use Nmap for MAC vendor detection
...
function identifyIoTDeviceType(vendor, deviceType, openPorts = []) {
    ...
    // === PRINTERS ===
    if (typeLower === 'printer' || openPorts.includes(9100) || openPorts.includes(631)) {
